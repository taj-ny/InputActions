...
InputEvent::InputEvent(const InputEventType &type, const InputDevice &sender)
    : m_type(type)
    , m_sender(sender)
{
}
...
MotionEvent::MotionEvent(const InputDevice &sender, const InputEventType &type, const QPointF &delta)
...
PointerButtonEvent::PointerButtonEvent(const InputDevice &sender, const Qt::MouseButton &button, const quint32 &nativeButton, const bool &state)
...
KeyboardKeyEvent::KeyboardKeyEvent(const InputDevice &sender, const uint32_t &nativeKey, const bool &state)
...
TouchpadClickEvent::TouchpadClickEvent(const InputDevice &sender, const bool &state)
...
TouchpadPinchEvent::TouchpadPinchEvent(const InputDevice &sender, const qreal &scale, const qreal &angleDelta)
...
TouchpadGestureLifecyclePhaseEvent::TouchpadGestureLifecyclePhaseEvent(const InputDevice &sender, const TouchpadGestureLifecyclePhase &phase,
...
TouchpadSlotEvent::TouchpadSlotEvent(const InputDevice &sender, const std::vector<TouchpadSlot> &fingerSlots)
