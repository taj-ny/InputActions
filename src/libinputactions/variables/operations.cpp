...
Q_LOGGING_CATEGORY(LIBINPUTACTIONS_VARIABLE_OPERATIONS, "libinputactions.variable.operations")
...
bool VariableOperations<T>::compare(const std::any &left, const std::any &right, const ComparisonOperator &comparisonOperator) const
{
    if (left.type() != typeid(T) || right.type() != typeid(T)) {
        qCWarning(LIBINPUTACTIONS_VARIABLE_OPERATIONS).noquote() << "Attempted illegal variable comparison (left: "
