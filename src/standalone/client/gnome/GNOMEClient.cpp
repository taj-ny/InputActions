...
{
    if (!GNOME_EXTENSIONS_DIR.exists()) {
        GNOME_EXTENSIONS_DIR.mkpath(".");
    }

    if (!GNOME_EXTENSION_DIR.exists()) {
        QFile::link(QString(INPUTACTIONS_DATA_DIR) + "/gnome/inputactions@inputactions.org", GNOME_EXTENSION_DIR.path());
    }
}
