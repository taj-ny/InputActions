...
void StandaloneInputBackend::evdevDeviceAdded(const QString &path)
{
    ...
    if (customDeviceProperties(device.get()).grab()) {
...
bool StandaloneInputBackend::handleEvent(InputDevice *sender, libinput_event *event)
{
    ...
    switch (type) {
        ...
        case LIBINPUT_EVENT_GESTURE_SWIPE_UPDATE: {
            ...
            switch (type) {
                case LIBINPUT_EVENT_GESTURE_HOLD_BEGIN:
                    touchpadHoldBegin(sender, fingers);
                    return false;
                case LIBINPUT_EVENT_GESTURE_HOLD_END:
                    touchpadHoldEnd(sender, cancelled);
                    return false;
                case LIBINPUT_EVENT_GESTURE_PINCH_BEGIN:
                ...
                case LIBINPUT_EVENT_GESTURE_PINCH_UPDATE: {
                    ...
                    touchpadPinchUpdate(sender, scale, angleDelta);
                    return false;
                }
                case LIBINPUT_EVENT_GESTURE_PINCH_END:
                    touchpadPinchEnd(sender, cancelled);
                    return false;
                case LIBINPUT_EVENT_GESTURE_SWIPE_BEGIN:
                    touchpadSwipeBegin(sender, fingers);
                    return false;
                case LIBINPUT_EVENT_GESTURE_SWIPE_UPDATE: {
                    ...
                    touchpadSwipeUpdate(sender, delta);
                    return false;
                }
                case LIBINPUT_EVENT_GESTURE_SWIPE_END:
                    touchpadSwipeEnd(sender, cancelled);
                    return false;
            }
            break;
        }
...
void StandaloneInputBackend::poll()
{
    ...
    for (auto &[device, data] : m_devices) {
        ...
        while (true) {
            ...
            if (!block || device->type() == InputDeviceType::Touchpad) {
                for (const auto &event : frame) {
...
libevdev_uinput *StandaloneInputBackend::outputDevice(const libinputactions::InputDevice *device) const
{
